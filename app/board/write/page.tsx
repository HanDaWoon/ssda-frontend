...
const Write = () => {
  ...
  const savePost = async () => {
    await fetch(`${API}/api/board/write`, {
      method: "POST",
      headers: {
      ...
      <div className="flex w-5/12 flex-col justify-center px-4 py-5 md:container md:mx-auto">
        <div className="relative mb-6">
          <label className="block">
            <span className="afFter:content-['*']  block text-sm font-medium text-slate-700 after:ml-0.5 after:text-red-500">
              제목
            </span>
            <input
              type="text"
        ...
        <button
          className="rounded bg-yellow-300 px-3 py-2 text-yellow-900 transition duration-200 hover:bg-yellow-400 hover:text-yellow-800"
          onClick={savePost}
        >
          저장
        </button>
